...
class KbMetaStore:
    ...
    def create(self, kb_id: str, name: str, description: str = "") -> dict[str, Any]:
        ...
        self._conn.execute(
            "INSERT INTO kb_meta(kb_id, name, description) VALUES(?,?,?)",
            (kb_id, name, description),
        )
        self._conn.commit()
        return {"kb_id": kb_id, "name": name, "description": description}

    def update(self, kb_id: str, name: str | None = None, description: str | None = None) -> bool:
        """改名称/描述（只更新传入的字段）。"""
    ...
    def list(self) -> list[dict[str, Any]]:
        rows = self._conn.execute(
            "SELECT kb_id, name, description, created_at FROM kb_meta ORDER BY created_at"
        ).fetchall()
        return [
            {"kb_id": r[0], "name": r[1], "description": r[2], "created_at": r[3]}
            for r in rows
        ]

    def get(self, kb_id: str) -> dict[str, Any] | None:
        row = self._conn.execute(
            "SELECT kb_id, name, description, created_at FROM kb_meta WHERE kb_id=?",
            (kb_id,),
        ).fetchone()
        if not row:
            return None
        return {"kb_id": row[0], "name": row[1], "description": row[2], "created_at": row[3]}
