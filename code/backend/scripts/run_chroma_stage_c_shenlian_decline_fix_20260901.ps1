...
# Stage C targeted gate for candidate: shenlian decline-claim fix (phase E round 1).
# attempt_id: chroma-stage-c-targeted8-shenlian-decline-fix-20260901
# This runner is a NEW file: it never overwrites the v2 report.
# Only the expected SHA256 values differ from run_chroma_stage_c_targeted8_20260901.ps1.

$backend = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$python = Join-Path $backend '.venv\Scripts\python.exe'
$testset = Join-Path $backend 'testsets\rag_real_quality_v2.yaml'
$report = Join-Path $backend 'reports\chroma-stage-c-targeted8-shenlian-decline-fix-20260901.json'
$envFile = Join-Path $backend '.env'
$attemptId = 'chroma-stage-c-targeted8-shenlian-decline-fix-20260901'
